...
export default function Services() {
  return (
    <section id="services" className="py-20 lg:py-28 bg-[var(--color-background)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h2 className="text-3xl sm:text-4xl font-bold text-[var(--color-foreground)] text-balance leading-tight max-w-lg">
              Services available to every resident
            </h2>
            <p className="text-[var(--color-muted)] text-base leading-relaxed max-w-sm text-pretty">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">
          {services.map(({ icon: Icon, title, desc, link, color }) => (
            <a
              key={title}
              href={link}
              className="group bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl p-6 hover:shadow-lg hover:-translate-y-0.5 hover:border-[var(--color-primary)]/20 transition-all duration-200"
            >
              <div className={`w-11 h-11 rounded-xl ${color} flex items-center justify-center mb-4`}>
                <Icon className="w-5 h-5" aria-hidden="true" />
              </div>
              <h3 className="text-[var(--color-foreground)] font-bold text-base mb-2">
                {title}
              </h3>
              <p className="text-[var(--color-muted)] text-sm leading-relaxed mb-4">
                {desc}
              </p>
              <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-[var(--color-primary)] group-hover:text-[var(--color-accent)] transition-colors duration-150">
                Learn more
        ...
        <div className="bg-[var(--color-primary)] rounded-2xl overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="p-8 lg:p-12">
              ...
              <a
                href="#"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white text-sm font-semibold rounded-lg transition-colors duration-150"
              >
                View Starter Guide
            ...
            {/* Right checklist */}
            <div className="bg-[var(--color-dark-surface)] p-8 lg:p-12">
              ...
              <ul className="flex flex-col gap-3">
                {studentResources.map((item, i) => (
                  <li key={item} className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-full bg-[var(--color-accent)]/20 text-[var(--color-accent)] text-xs font-bold flex items-center justify-center flex-shrink-0">
                      {i + 1}
                    </span>
              ...
              <a
                href="#"
                className="inline-flex items-center gap-1.5 mt-6 text-sm font-semibold text-[var(--color-accent)] hover:text-white transition-colors duration-150"
              >
