...
export default function HospitalsClient({
  ...
  initialHospitals: SanityInstitution[];
}) {
  const [searchQuery, setSearchQuery] = useState("");

  const filtered = (initialHospitals || []).filter(
    (hosp) =>
        ...
        <div className="bg-[#0d2238] text-white pt-24 pb-16 lg:pt-32 lg:pb-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="text-white/70 max-w-2xl text-lg leading-relaxed">
              Find emergency care, walk-in community clinics, diagnostic
              laboratories, and pediatric specialists in Township.
            </p>
          </div>
        </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-muted mb-4">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <a
                          href="https://hospital.township.org"
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-primary hover:underline font-medium"
                        >
                          hospital.township.org
                        </a>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {hosp.tags?.map((tag) => (
                        <span
                          key={tag}
                          className="px-2.5 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 text-[11px] font-medium rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
