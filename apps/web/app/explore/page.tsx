...
const categories: { id: Category; label: string; icon: React.ElementType; link: string; color: string; bg: string }[] = [
  { id: "all", label: "All Places", icon: Star, link: "/explore", color: "text-primary", bg: "bg-primary-light" },
  { id: "Dining", label: "Dining & Cafes", icon: UtensilsCrossed, link: "/explore/dining", color: "text-orange-500", bg: "bg-orange-50" },
  { id: "Shopping", label: "Shopping", icon: ShoppingBag, link: "/explore/shopping", color: "text-blue-500", bg: "bg-blue-50" },
  { id: "Parks", label: "Parks & Recreation", icon: TreePine, link: "/explore/parks", color: "text-emerald-600", bg: "bg-emerald-50" },
  { id: "Stay", label: "Accommodation", icon: Hotel, link: "/explore/accommodation", color: "text-purple-600", bg: "bg-purple-50" },
];
      ...
      <main className="bg-background min-h-screen">
        ...
        <div className="bg-[#0d2238] text-white pt-24 pb-16 lg:pt-32 lg:pb-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p className="text-white/70 max-w-2xl text-lg leading-relaxed">
              Discover local favorites, hidden treasures, parks, dining options, and accommodation options in our beautiful city.
            </p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between border-b border-border pb-6 mb-8">
            {/* Category pills */}
            <div className="flex flex-wrap gap-2 w-full md:w-auto" role="tablist" aria-label="Explore categories">
          ...
          {/* Places Grid */}
          <h2 className="text-xl font-bold text-foreground mb-4">All Curated Places</h2>
          {filtered.length > 0 ? (
              ...
              {filtered.map((place) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: "linear-gradient(to top, rgba(13,34,56,0.95) 0%, rgba(13,34,56,0.3) 60%, transparent 100%)" }}
                    aria-hidden="true"
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white/80 font-semibold text-xs">{place.rating}</span>
                      <span className="text-white/40 text-xs">({place.reviews} reviews)</span>
                    </div>
                    <div className="mt-4 pt-3 border-t border-white/10 flex justify-between items-center">
                      <div className="flex gap-1">
                        {place.tags.slice(0, 2).map((tag) => (
                          <span key={tag} className="text-white/50 text-[10px] bg-white/10 px-2 py-0.5 rounded">
                            {tag}
                          </span>
                        ))}
                      </div>
            ...
            <div className="text-center py-12 bg-surface rounded-xl border border-border">
              <p className="text-muted text-base">No places found matching your search.</p>
              <button
