...
export default function ItemDetailPage({ params }: PageProps) {
  ...
  useEffect(() => {
    // Reset quantities & selected image if product changes
    setQuantity(1);
    setSuccessMessage("");
  ...
  if (!product) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-8 text-center min-h-[60vh]">
        ...
        <Link
          href="/"
          className="mt-4 px-5 py-2 bg-emerald-500 hover:bg-emerald-400 text-black text-sm font-semibold rounded-md transition shadow-xs"
        >
          Back to Marketplace
        </Link>
      </main>
  ...
  // Set up multiple images array fallback (if Sanity doesn't return any array, use main imageUrl)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
      ...
      <div className="mb-6">
        <Link
          href="/"
          className="inline-flex items-center gap-2 text-sm font-bold text-muted hover:text-emerald-500 transition-colors group"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left: Product Image Gallery & Descriptions */}
        <div className="lg:col-span-8 space-y-8">
          <div className="bg-surface border border-border rounded-xl p-6 md:p-8 space-y-6 shadow-xs">

            {/* Product Header Row */}
            <div className="space-y-2">
              <span className="px-2.5 py-1 rounded-md text-[10px] uppercase font-black bg-emerald-500/10 text-emerald-500 border border-emerald-500/10">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs font-bold text-muted pt-1">
                ...
                {product.stock > 0 ? (
                  <span className="text-emerald-600 bg-emerald-50/5 px-2.5 py-0.5 rounded-full border border-emerald-500/20 flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    In Stock ({product.stock} units remaining)
                  </span>
                ) : (
                  <span className="text-red-600 bg-red-50/5 px-2.5 py-0.5 rounded-full border border-red-500/20 flex items-center gap-1">
            ...
            <div className="space-y-4">
              ...
              {/* Multiple thumbnails rendering at the bottom */}
              {productImages.length > 1 && (
                <div className="space-y-2">
                  <span className="block text-[10px] font-black text-foreground-secondary uppercase tracking-widest">
                    Available Images ({productImages.length})
                  </span>
                    ...
                    {productImages.map((imageSource, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={`relative w-20 h-16 sm:w-24 sm:h-20 rounded-lg overflow-hidden border-2 transition-all duration-200 shrink-0 bg-muted hover:scale-105 active:scale-95 ${
                            isSelected
                              ? "border-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.3)] scale-[1.02]"
                        ...
                        >
                          ...
                          {isSelected && (
                            <div className="absolute inset-0 bg-emerald-500/10 flex items-center justify-center">
                              <span className="bg-emerald-500 text-black text-[9px] font-black px-1 py-0.5 rounded-sm uppercase tracking-wide">
                                Active
                              </span>
                            </div>
                          )}
        ...
        <div className="lg:col-span-4 sticky top-24">
          <div className="bg-surface border border-border rounded-xl p-6 shadow-md space-y-4">
            ...
            {product.deposit && (
              <div className="bg-emerald-500/10 border border-emerald-500/15 rounded-lg p-3.5 space-y-1">
                <h4 className="font-bold text-emerald-500 text-xs flex items-center gap-1.5">
            ...
            {/* Alerts */}
            {successMessage && (
              <div className="bg-green-500/10 border border-green-500/20 text-green-700 text-xs p-3.5 rounded-lg font-semibold space-y-1.5">
                ...
                <Link
                  href="/cart"
                  className="mt-2 block text-center py-1.5 bg-emerald-500 hover:bg-emerald-400 text-black text-[11px] font-bold rounded-md"
                >
            ...
            {product.stock <= 0 ? (
              <div className="bg-red-500/5 border border-red-500/10 text-red-600 text-xs p-3.5 rounded-lg font-medium text-center">
              ...
              <form onSubmit={handleAddToCartSubmit} className="space-y-4">
                ...
                {quantity > 1 && (
                  <div className="bg-background rounded-lg p-3 space-y-2 text-xs border border-border font-semibold">
                    ...
                    <div className="border-t border-border pt-1.5 flex justify-between font-bold text-emerald-500">
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-black rounded-lg uppercase tracking-wider transition shadow-sm flex items-center justify-center gap-1.5 cursor-pointer"
                >
